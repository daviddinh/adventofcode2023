...
for (let i = 0; i < input.length; i++) {
  ...
  while (sequence.length > 1) {
    let numberToTake = sequence[sequence.length - 1][0]
    sequence[sequence.length - 2] = [sequence[sequence.length - 2][0] - numberToTake, ...sequence[sequence.length - 2]]
    sequence = sequence.slice(0, sequence.length - 1)
  }
  nextElements.push(sequence[0][0])
}
